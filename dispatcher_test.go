...
func (s *InternalCommandBusSuite) TestShouldHandleCommand(c *C) {
	err := s.bus.AddCommandHandler(s.stubhandler, &SomeCommand{})
	c.Assert(err, IsNil)
...
func (s *InternalCommandBusSuite) TestDuplicateHandlerRegistrationReturnsAnError(c *C) {
	err := s.bus.AddCommandHandler(s.stubhandler, &SomeCommand{}, &SomeCommand{})
...
func (s *InternalCommandBusSuite) TestCanRegisterMultipleCommandsForTheSameHandler(c *C) {
	...
	err := s.bus.AddCommandHandler(s.stubhandler, &SomeCommand{}, &SomeOtherCommand{})
	c.Assert(err, IsNil)
